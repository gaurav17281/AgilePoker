package com.ga.agile;

public class Constants {

	public static final String STRING_HALF = "1/2";
	public static final String STRING_ONE = "1";
	public static final String STRING_TWO = "2";
	public static final String STRING_THREE = "3";
	public static final String STRING_FIVE = "5";
	public static final String STRING_EIGHT = "8";
	public static final String STRING_THIRTEEN = "13";
	public static final String STRING_TWENTY = "20";
	public static final String STRING_FORTY = "40";
	public static final String STRING_HUNDRED = "100";
	public static final String STRING_UNKNOWN = "?";
	public static final String STRING_COFFEE = "COFFEE";

	public static final int DEFAULT_BG_COLOR = 0xFFFFFF; // WHITE
	public static final int DEFAULT_FG_COLOR = 0x000000; // BLACK
	// bundle keys
	public static final String KEY_CARD_TEXT = "_KEY_CARD_TEXT_";
	public static final String KEY_COLOR_CODE = "_KEY_COLOR_CODE_";
	public static final String KEY_WHICH_MODE = "_KEY_WHICH_MODE_";

	// color keys
	public static final int BG_COLOR_SELECTOR = 1;
	public static final int FG_COLOR_SELECTOR = 2;
	
	// Shared Preference Keys
	public static final String AGILE_POKER_PREF = "_AGILE_POKER_PREF_";
	public static final String BG_COLOR = "_BACKGROUND_COLOR_";
	public static final String FG_COLOR = "_FORGROUND_COLOR_";

	public static final int POKER_CARD_UPDATE_DELAY = 1000; // in milli seconds
	public static final int POKER_CARD_LIFE_TIME = 10; // in seconds
}
